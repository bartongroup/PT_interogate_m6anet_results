...
def filter_m6anet_files(m6anet_dir, mapped_data, match_criteria="original"):
    ...
    combined_significant = pd.concat(all_indiv_significant, ignore_index=True)
    combined_non_significant = pd.concat(all_indiv_non_significant, ignore_index=True)
...
def calculate_summary_stats(filtered_main, indiv_significant, indiv_non_significant, output_file):
    ...
    output_file.write(f"Total retained introns failing modification threshold:\t{retained_introns_without_modification}\n")
